```
1.当关键字为空时赋默认值
2.创建NativeSearchQueryBuilder对象
3.根据NativeSearchQueryBuilder对象addAggregation添加规则AggregationBuilders分类
4.使用esTemplate.queryForPage构造的AggregatedPage对象获取分组结果得到StringTerms对象
5.封装方法获取集合存入map
6.通过QueryBuilders.boolQuery();创建过滤对象,过滤方法是filter,NativeSearchQueryBuilder对象用withFilter方法融合boolQuery
7.NativeSearchQueryBuilder对象用withPageable方法分页,使用withSort排序
```
